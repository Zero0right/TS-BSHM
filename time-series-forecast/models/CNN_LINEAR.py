...
class Model(nn.Module):
    def __init__(
        ...
        self.tcn_output_dim=configs.enc_in*configs.cnn_output
        self.d_model=configs.d_model
        encoder_dim = (
                self.seq_len * self.enc_in
        )
        ...
        #         output_dim=configs.seq_len*configs.enc_in,
        ...
        # self.encoders = nn.Sequential(
        #     nn.Linear(self.seq_len+1, self.hidden_size)
        # )
        self.decoders = nn.Sequential(
            nn.Linear(configs.cnn_output, self.pred_len)
        )
        # self.linear=nn.Linear(self.seq_len+1,self.pred_len)
    ...
    def forward(
        ...
        # encoded = encoded.view(x.shape[0], self.seq_len, -1)
        x_lookback_cnn_features=x_lookback_cnn_features.view(x.shape[0], -1, self.enc_in)
        decoded = self.decoders(x_lookback_cnn_features.permute(0, 2, 1)).permute(0, 2, 1)
