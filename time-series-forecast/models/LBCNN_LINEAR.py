...
class Model(nn.Module):
    ...
    def forward(
        ...
        encoded = encoded.view(x.shape[0], self.seq_len+1, -1)
        # print("before encoders :",encoded.shape)
        # encoded = self.encoders(encoded.permute(0, 2, 1)).permute(0, 2, 1)
        # decoded = self.decoders(encoded.permute(0, 2, 1)).permute(0, 2, 1)
        decoded = self.linear(encoded.permute(0, 2, 1)).permute(0, 2, 1)
        ...
        y = y.view(-1, self.pred_len, self.output_dim)
        seq_last = x[:, -1:, :].detach()
        y = y + seq_last
        # print("y:", y.shape)
        return y
