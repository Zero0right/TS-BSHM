...
class Model(nn.Module):
    ...
    def __init__(self,configs):
        ...
        for i in range(num_layers): #num_layers:4 384_288:7
            ...
            self.res_blocks_list_encoder.append(res_block)
        # self.res_blocks = nn.ModuleList(self.res_blocks_list)
        self.encoder=nn.ModuleList(self.res_blocks_list_encoder)
        self.decoder=nn.LSTM(self.encoder_output_dim, self.encoder_output_dim*2, configs.e_layers)
        self.Linear = nn.Linear(self.encoder_output_dim*2, self.target_length)
    ...
    def forward(self, x_in, x_mark, y_true, y_mark):
        ...
        x = self.Linear(x[0]).permute(0, 2, 1)
