...
class TCN_ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.pad(x, (left_padding, 0),mode='replicate') #对输入张量进行填充
        # print("---first step")
        ...
        # second step
        # print("---second step")
        x = F.pad(x, (left_padding, 0),mode='replicate') #填充
        # print("left_padding:",x.shape)
        x = self.conv2(x) #卷积
        # print("conv:",x.shape)
...
class BackBone(nn.Module):
    def __init__(self, configs):
        ...
        # self.conv_ffn = ConvFFN(self.enc_in, self.patch_len, r=1, one=True)
        self.conv_ffn=ConvFFN(self.enc_in,self.patch_len,r=1,one=False)
        # 4
        # self.mlp = Mlp(patch_len * (patch_num-1), pred_len * 2, pred_len)
        self.e_kan = KAN([patch_len * (patch_num - 1), 10, pred_len])

    def forward(self, x): # B, L, D -> B, H, D
        B, _, D = x.shape
        # patching
        # 初始化一个张量来存储所有 patch
        patched_tensor = torch.zeros(self.batch_size, self.patch_num-1, self.patch_len, self.enc_in).cuda()
        # 进行 patching 操作
        ...
        z=patched_tensor.permute(0, 3, 1, 2) #B,pn,pl,D -> B,D,pn,pl
        z=z.reshape(z.shape[0]*z.shape[1],z.shape[2],z.shape[3]) #B*D,pn,pl
        for res_block in self.res_blocks_list:
            z = res_block(z) # B*D,pn,pl
        z=z.reshape(self.batch_size,self.enc_in,self.patch_num-1,self.patch_len) # B,D,pn,pl
        z=z.view(self.batch_size, self.patch_len * self.enc_in, self.patch_num-1) # B,pl*D,pn
        z=self.conv_ffn(z) # B,pl*D,pn
        # 首先使用 view 重塑张量，使其形状变为 [B, D, pl, pn]
        z = z.view(self.batch_size, self.enc_in, self.patch_num-1, self.patch_len)
        # 然后使用 permute 进行转置，使形状变为 [B, D, pn, pl]
        z = z.permute(0, 1, 3, 2)
        # 最后再次使用 view 重塑张量，使其形状变为 [B, D, pn*pl]
        z = z.reshape(self.batch_size, self.enc_in, (self.patch_num - 1) * self.patch_len)
        z_kan = self.e_kan(z)
        return z_kan.permute(0, 2, 1)
