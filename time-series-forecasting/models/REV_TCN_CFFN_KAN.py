...
class Model(nn.Module):
    def __init__(self,configs):
        ...
        self.conv_ffn = ConvFFN(configs.enc_in, 1, r=1, one=False)
        ...
        self.e_kan = KAN([self.target_length//4, 10, self.target_length])
        # self.e_kan = KAN([self.input_size,10, self.target_length])
    ...
    def forward(self, x_in, x_mark, y_true, y_mark):
        ...
        for res_block in self.res_blocks_list:
            x = res_block(x)
        x=self.conv_ffn(x.permute(0, 2, 1)).permute(0, 2, 1)
        # print("TCN:",x.shape)
        y = self.MLP(x.permute(0, 2, 1)).permute(0, 2, 1)
        # y = self.e_kan(x.permute(0, 2, 1)).permute(0, 2, 1)
        # print("MLP:",y.shape)
        # y = self.Linear(x)
        y = self.rev(y, 'denorm') # B, L, D -> B, H, D
        return y
